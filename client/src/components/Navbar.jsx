...
const NavbarItem = ({title, classProps}) => (
	<li className={`mx-4 cursor-pointer hover:text-[#e6e6e6] ${classProps}`}>{title}</li>
);
	...
	return (
		<nav className="w-full flex md:justify-center justify-between items-center p-4">
			...
			<ul className="text-white md:flex hidden list-none flex-row justify-between items-center flex-initial">
				{NAV_ITEMS.map((item, i) => (<NavbarItem title={item} key={item + i} />))}
			...
			<div className="flex relative">
				...
				{toggleMenu && (
					...
					>
						...
						{[...NAV_ITEMS, 'Login'].map((item, i) => (<NavbarItem classProps="my-2 text-lg" title={item} key={item + i} />))}
					</ul>
				)}
			</div>
		</nav>
